...
import CreateVideos from "./CreateVideo";
...
export default function CoursesVideos() {
  ...
  const getVideoInfo = async(e, id, data) => {
    e.preventDefault()
    if(data){
      setLoading(true)
      setLocalModuleId(data)
      sessionStorage.setItem("moduleid", data)
      let response = await new CoursesAPI().getVideoInformation(id, data)
      setLoading(false)
      if(response.ok){
  ...
  const getCourseUnitInformation = async(e) => {
    setLoading(true)
    let response = await new ClassesAPI().getClassUnit(id)
    setLoading(false)
    if(response.ok){
  ...
  const deleteCourseVideo = async(vidID) => {
    ...
    let response = await new ClassesAPI().deleteVideo(id, videoId)
    setLoading(false)
    if(response.ok){
  ...
  return (
    <ClassSideNavigation>
      <ClassBreedCrumbs title={videoName} clicked={() => clickedTab()}/>
     {showVideos ?
        ...
        {moduleInfo?.map((item, index) => {
          return(
            ...
            <Accordion.Item eventKey={item.id}> 
              <Accordion.Header onClick={(e) => {getVideoInfo(e, item.courseId, item.id)}}>
                <span className="unit-title">{item.moduleName} 
                {/* <Button className="btn-create-class" variant="link" onClick={handleOpenCreateVideoModal}><i className="fa fa-plus"></i> Add Video</Button> */}
                </span>
              </Accordion.Header>
              <Accordion.Body>
